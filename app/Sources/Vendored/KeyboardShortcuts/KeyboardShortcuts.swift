...
// Source: Sources/KeyboardShortcuts/KeyboardShortcuts.swift
// Local modifications:
//   - Added `@MainActor` to `repeatingKeyDownEvents(for name:)` (the Name
//     overload). Upstream compiles with Swift 6.2 + `defaultIsolation
//     (MainActor.self)` so the whole file is implicitly MainActor; we build
//     in Swift 5 mode (project SWIFT_VERSION=5.9) where that file-wide
//     isolation isn't applied. The inner `RepeatState` class is `@MainActor`
//     and `let repeatState = RepeatState()` requires the enclosing function
//     to be MainActor too. Annotating the function is the smallest possible
//     change. Out of scope this loop: the polling logic still needs Loop 32+
//     auditing for actor-isolation drift in 5.9 mode.
