...
// Source: Sources/KeyboardShortcuts/HotKey.swift
// Local modifications:
//   - Replaced `isolated deinit` with plain `deinit`. Upstream uses Swift 6.1's
//     `IsolatedDeinit` experimental feature (compile flag
//     `-enable-experimental-feature IsolatedDeinit`); cc-dashboard builds in
//     Swift 5 language mode without that flag. Under 5.9 mode `HotKeyCenter`
//     is nonisolated so no isolation is required to call `unregister(self)`
//     from deinit. Behavior matches upstream effective semantics on macOS 14.
...
final class HotKey {
	...
	*/
	...
	deinit {
		// VENDORED MODIFICATION: dropped `isolated` keyword — see header comment.
