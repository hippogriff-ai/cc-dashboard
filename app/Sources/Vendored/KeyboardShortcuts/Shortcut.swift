...
// Source: Sources/KeyboardShortcuts/Shortcut.swift
// Local modifications:
//   - Replaced `"space_key".localized.capitalized` with literal "Space" in
//     `SpecialKey.presentableDescription` because this vendored copy strips
//     the upstream `.lproj` localization bundles (English only). Loop 32 in
//     cc-dashboard's plan defers Recorder UI + localization; this is the only
//     site in the non-Recorder code path that hits a localized string.
...
extension SpecialKey {
	fileprivate var presentableDescription: String {
		...
		case .home:
			"↖"
		case .space:
			// VENDORED MODIFICATION: upstream had `"space_key".localized.capitalized`
			// which depended on the SwiftPM `.module` bundle being present. We
			// strip localization (Recorder UI deferred) so use the literal English.
			"Space"
		case .tab:
			"⇥"
		case .pageUp:
