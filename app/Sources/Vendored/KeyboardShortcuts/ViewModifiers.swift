...
// Source: Sources/KeyboardShortcuts/ViewModifiers.swift
// Local modifications: none. All referenced APIs (`events(for:)`,
// `events(_:for:)`, `EventType`, `Name.shortcut`, `Shortcut.toSwiftUI`,
// `.shortcutByNameDidChange`, `.recorderActiveStatusDidChange`) are already
// vendored. The file is gated on `macOS 12+` / `macOS 12.3+` which sits below
// our deployment target of `macOS 14`, so the availability checks always pass.
