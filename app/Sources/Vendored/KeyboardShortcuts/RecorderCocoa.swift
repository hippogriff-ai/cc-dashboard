...
// Source: Sources/KeyboardShortcuts/RecorderCocoa.swift
// Local modifications:
//   - Replaced `isolated deinit` with plain `deinit` on `RecorderCocoa`.
//     Upstream uses Swift 6.1's `IsolatedDeinit` experimental feature; this
//     project builds in Swift 5 mode where `isolated` is not a valid deinit
//     keyword. The semantics in our case are equivalent: the observers being
//     removed are `NSObjectProtocol` tokens which can be safely removed from
//     any thread (NotificationCenter is documented thread-safe).
//   - The seven `.localized` call sites are kept verbatim. Pairs with the
//     `Utilities.swift` modification (Loop 32) that extends `String.localized`
//     with a literal `[String: String]` dictionary mirroring upstream's
//     `en.lproj/Localizable.strings`. Keys: `record_shortcut`, `press_shortcut`,
//     `keyboard_shortcut_used_by_menu_item`, `keyboard_shortcut_disallowed`,
//     `keyboard_shortcut_used_by_system`, `keyboard_shortcuts_can_be_changed`,
//     `ok`, `force_use_shortcut`.
