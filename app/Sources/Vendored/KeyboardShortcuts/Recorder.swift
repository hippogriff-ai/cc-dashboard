...
// Source: Sources/KeyboardShortcuts/Recorder.swift
// Local modifications: none. The Recorder type is a thin SwiftUI wrapper over
// `RecorderCocoa`; all upstream APIs it touches (Shortcut, ValidationResult,
// ConflictPolicy, the `keyboardShortcutsConflictPolicy` environment value) are
// already vendored. The `#Preview` blocks at the file foot use the literal
// string `"record_shortcut"` as a label — that's a localization-key string in
// the upstream code that gets resolved by `LocalizedStringKey` initializer
// lookup; in our app target there is no `.strings` resource, so it would
// render as the literal "record_shortcut". The previews are debug-only / not
// shipped, so this is left as-is rather than patched.
